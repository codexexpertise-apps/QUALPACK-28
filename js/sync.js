...
function getSupabaseHeaders() {
  ...
  const siteId = (typeof QUALPACK_SITE_ID !== 'undefined' && QUALPACK_SITE_ID) || '';
  ...
  return {
    ...
    'x-qualpack-site-id': siteId,
    'x-qualpack-site-key': siteKey
  };
}
...
function getQualPackSiteIdForSync() {
  try {
    if (typeof QUALPACK_SITE_ID !== 'undefined' && QUALPACK_SITE_ID) return QUALPACK_SITE_ID;
...
function mapPeseeForSupabase(r) {
  return {
    id: r.id,
    site_id: r.site_id || getQualPackSiteIdForSync(),
    type: r.type || 'pesee',
    cli: r.cli || null,
    prod: r.prod || null,
    of: r.of || null,
